...
import com.passion.ballulala.dto.ItemDto;
...
public interface UserItemRepo extends JpaRepository<UserItem, Long> {
    ...
    @Query("SELECT new com.passion.ballulala.dto.UserItemBuyListDto(ui.item.id, ui.user.point, ui.item.img)" +
        "FROM UserItem ui WHERE ui.item.id = ?1")
    UserItemBuyListDto findByItemId(Long itemId);
}
